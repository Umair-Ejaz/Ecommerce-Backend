...
export const createOrderItem = async (req, res) => {
  try {
    ...
    if (!orderId || !productId || !qty) {
      return res.status(400).json({ error: "orderId, productId, and qty are required" });
    }
  ...
  try {
    const items = await OrderItem.findAll({
      include: [
        ...
        { model: Order, attributes: ["id", "name", "email", "address", "total"], as: "Order" }
      ],
    });
    res.json(items);
  } catch (err) {
...
export const getOrderItemById = async (req, res) => {
  try {
    const item = await OrderItem.findByPk(req.params.id, {
      include: [
        { model: Product, attributes: ["id", "name", "price", "imageUrl"] },
        { model: Order, attributes: ["id", "name", "email", "address", "total"] },
      ],
    });
